...
-->
<!--
  Changes by Oscar Fonts <oscar.fonts@geomati.co> (2015-06-30):

   * Remove Representations section
   * In Resource index: show base url as first item, then relative paths
   * In Resource descriptions: remove OPTIONS verb; add handy link to resource

   Original work from: https://github.com/mnot/wadl_stylesheets
-->
<xsl:stylesheet 
...
<xsl:template match="/wadl:application">   
    <html>
        <head>
            <title>
                <xsl:choose>
                    ...
                    <xsl:otherwise>OTP v.0.18 REST API</xsl:otherwise>
                </xsl:choose>                 
            </title>
            <style type="text/css">
                body {
        ...
        <body>
            ...
            <ul>
                <li><a href="#resources"><xsl:value-of select="wadl:resources/@base"/></a>
    ...
    <xsl:choose>
        ...
        <xsl:otherwise>
            ...
            <xsl:variable name="link">
               <xsl:value-of select="substring(/wadl:resources/@base, 1, string-length(/wadl:resources/@base) - 1)"/><xsl:value-of select="$name"/>
           </xsl:variable>
           <div class="resource">
            <h3 id="{$id}">
                <xsl:choose>
